...
ExpansionHeader::ExpansionHeader(DeviceConfig *config, QWidget *parent)
    : QDialog(parent), ui(new Ui::ExpansionHeader), _config(config) {
  ui->setupUi(this);
...
ExpansionHeader::~ExpansionHeader() { delete ui; }
...
void ExpansionHeader::accept(void) {
  _config->setExpHeaderParams(ui->modeBox->currentIndex(), ui->Param1->value(),
                              ui->Param2->value());
  QDialog::accept();
}
