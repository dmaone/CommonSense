...
LayerConditionWatcher::LayerConditionWatcher(
    ...
    QComboBox *layerCombo, QObject *parent)
...
void LayerConditions::init(void) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    for (int j = 0; j < config->numLayers; j++)
      layerCombo->addItem(j == 0 ? "Base Layer" : QString("Layer %1").arg(j),
                          j);
    ...
    layerConditionWatchers.push_back(new LayerConditionWatcher(
        config, i, fn1Check, fn2Check, fn3Check, fn4Check, layerCombo, this));
  }
