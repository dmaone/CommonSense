...
FirmwareLoader::FirmwareLoader(QObject *parent) : QObject(parent),
    bootloaderMode(false), firmware(NULL), lastCommand(BR_CYRET_SUCCESS)
{
...
bool FirmwareLoader::_checkCompatibility(Bootloader_packet_t *packet)
{
    ...
    if (siliconId != firmware->siliconId || packet->payload[4] != firmware->siliconRevision)
    {
        qCritical() << "Firmware from incompatible hardware! Please use correct firmware!";
        return true;
    }
    qInfo().nospace() << "Remote bootloader version " << (uint8_t)packet->payload[5] << "." << (uint8_t)packet->payload[6]  << "." << (uint8_t)packet->payload[7];
    // MAYBE FLASH SIZE CHECK
    // Checking flash size gets ERR_DATA, so some other day. We check for siliconId already, oughtta be enough.
    // Let's just upload the row instead.
...
bool FirmwareLoader::eventFilter(QObject *obj __attribute__((unused)), QEvent *event)
{
...
//    qInfo().noquote() << QString::number(packet->raw[0], 16) << QString::number(packet->raw[1], 16) << QString::number(packet->raw[2], 16) << QString::number(packet->raw[3], 16)
//            << QString::number(packet->raw[4], 16) << QString::number(packet->raw[5], 16) << QString::number(packet->raw[6], 16) << QString::number(packet->raw[7], 16)
//            << QString::number(packet->raw[8], 16) << QString::number(packet->raw[9], 16) << QString::number(packet->raw[10], 16) << QString::number(packet->raw[11], 16)
//            << QString::number(packet->raw[12], 16) << QString::number(packet->raw[13], 16) << QString::number(packet->raw[14], 16) << QString::number(packet->raw[15], 16);
...
//            break;
        ...
        case BCMD_ExitBootloader:
            ...
            break;
        default:
            qInfo() << "Received response" << (BootloaderVerb)packet->command << "for command" << lastCommand;
    }
...
void FirmwareLoader::start(void)
{
    ...
    QMessageBox::StandardButton result = QMessageBox::question(NULL,
            "Are you sure?",
            QString("About to flash %1\n\nYou will lose communication with the device until reset or firmware update!").arg(settings.value(FIRMWARE_FILE_KEY).toString()),
