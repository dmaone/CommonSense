...
If you have PSoC Creator, Qt+Qt creator, CY8CKIT-059 prototype board and an IBM keyboard with 16x8 matrix (that includes practically any model F, but not beamsprings) - you may try it. You may also fry the controller doing it. If you break it - you own all the parts.

If you are lucky^W^W do everything correctly - you will see the USB device with 3 interfaces and ADC readings in the utility (or wireshark) when you press the keys.
...
4.1) Don't forget to momentarily pull the ping 2.3 so that the board is in bootloader. It will not be visible otherwise.
...
Now press the buttons on the keyboard. You should see ADC readings.

That's all, folks.
The utility will soon be able to program the keyboard (settings and keymaps).
The firmware will be able to send actual scancodes!
