/*******************************************************************************
* File Name: SupervisoryUART_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_Start
********************************************************************************
*
* Summary:
...
void SupervisoryUART_IntClock_Start(void) 
{
    ...
    SupervisoryUART_IntClock_CLKEN |= SupervisoryUART_IntClock_CLKEN_MASK;
	SupervisoryUART_IntClock_CLKSTBY |= SupervisoryUART_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_Stop
********************************************************************************
*
* Summary:
...
void SupervisoryUART_IntClock_Stop(void) 
{
    ...
    SupervisoryUART_IntClock_CLKEN &= (uint8)(~SupervisoryUART_IntClock_CLKEN_MASK);
	SupervisoryUART_IntClock_CLKSTBY &= (uint8)(~SupervisoryUART_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SupervisoryUART_IntClock_CLKEN & SupervisoryUART_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SupervisoryUART_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SupervisoryUART_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SupervisoryUART_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SupervisoryUART_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SupervisoryUART_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SupervisoryUART_IntClock_GetSourceRegister();
    uint16 oldDivider = SupervisoryUART_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SupervisoryUART_IntClock_CLKEN & SupervisoryUART_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(SupervisoryUART_IntClock_DIV_PTR, clkDivider);
                SupervisoryUART_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                SupervisoryUART_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(SupervisoryUART_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SupervisoryUART_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(SupervisoryUART_IntClock_DIV_PTR, clkDivider);
				SupervisoryUART_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SupervisoryUART_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SupervisoryUART_IntClock_MOD_SRC |= modeBitMask & (uint8)SupervisoryUART_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SupervisoryUART_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SupervisoryUART_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SupervisoryUART_IntClock_MOD_SRC & (uint8)(SupervisoryUART_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SupervisoryUART_IntClock_GetDividerRegister();
    uint8 oldSrc = SupervisoryUART_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SupervisoryUART_IntClock_MOD_SRC |= CYCLK_SSS;
        SupervisoryUART_IntClock_MOD_SRC =
            (SupervisoryUART_IntClock_MOD_SRC & (uint8)(~SupervisoryUART_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SupervisoryUART_IntClock_MOD_SRC & SupervisoryUART_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SupervisoryUART_IntClock_PHASE = clkPhase & SupervisoryUART_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SupervisoryUART_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SupervisoryUART_IntClock_PHASE & SupervisoryUART_IntClock_PHASE_MASK;
}
