...
static cystatus Boot_WriteFlashByte(const uint32 address, const uint8 inputValue) CYLARGE \
                               ;
#endif /*(CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)*/

/*******************************************************************************
* Function Name: Boot_Load
****************************************************************************//**
*
* \brief
...
void Boot_Load(void) 
{
    ...
    Boot_SET_RUN_TYPE(Boot_SCHEDULE_BTLDR);
...
#if (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)
/*******************************************************************************
* Function Name: Boot_GetActiveApplication
****************************************************************************//**
*
* \brief
...
{   
    uint8 result = Boot_MD_BTLDB_ACTIVE_NONE;
    
    if (0u != Boot_GET_CODE_DATA( \
              Boot_MD_BTLDB_ACTIVE_OFFSET(Boot_MD_BTLDB_ACTIVE_0))) 
    {
        result = Boot_MD_BTLDB_ACTIVE_0;
    }
    else if (0u != Boot_GET_CODE_DATA( \
                   Boot_MD_BTLDB_ACTIVE_OFFSET(Boot_MD_BTLDB_ACTIVE_1)))
    {
        result = Boot_MD_BTLDB_ACTIVE_1;    
    }
    else
    {
...
/*******************************************************************************
* Function Name: Boot_SetActiveApplication
****************************************************************************//**
*
* \brief 
                ...
                
{
    ...
    if (appId > Boot_MD_BTLDB_ACTIVE_1)
    {
    ...
    else
    {
        ...
        if (1u == Boot_GET_CODE_DATA(Boot_MD_BTLDB_ACTIVE_OFFSET(appId)))
        {
            result = CYRET_BAD_PARAM;
        }
        else
        {
            ...
            for(idx = 0u; idx < Boot_MAX_NUM_OF_BTLDB; idx++)
            {
                result |= Boot_WriteFlashByte((uint32) Boot_MD_BTLDB_ACTIVE_OFFSET(idx), \
                                                                                            (uint8)(idx == appId));
            }
        }
    }
...
/*******************************************************************************
* Function Name: Boot_WriteFlashByte
****************************************************************************//**
*
* \brief 
                               ...
                               
{
    ...
    {
        rowData[idx] = (uint8)Boot_GET_CODE_DATA(baseAddr + idx);
    }
...
void Boot_SetFlashByte(uint32 address, uint8 runType) 
{
    ...
    for (idx = 0u; idx < CYDEV_FLS_ROW_SIZE; idx++)
    {
        rowData[idx] = Boot_GET_CODE_DATA(baseAddr + idx);
    }
